...
//! # Dropdown and Combobox Popup Menu Widget Builder
//!
//! Provides a standardized, hardware-accelerated GPU SDF dropdown menu builder
...
mod tests {
    ...
    #[test]
    fn test_combobox_popup_builder_and_tagging() {
        let mut tree = UiTree::new();
        let root = tree.create_node();
        let _ = tree.set_root(root);

        let trigger = Rect::new(200.0, 100.0, 150.0, 26.0);
        let items = ["60 FPS", "120 FPS", "Uncapped"];

        let frame = ComboboxPopupBuilder::new(trigger)
            .items(&items)
            .selected_index(Some(1))
            .cursor_pos(Point::new(220.0, 155.0))
            .build(&mut tree, root);

        assert_eq!(frame.item_ids.len(), 3);
        assert_eq!(frame.popup_rect.x, 200.0);
        assert_eq!(frame.popup_rect.y, 128.0); // 100.0 + 26.0 + 2.0

        // Verify that each item node has WidgetRole::DropdownItem, UiLayer::Popup, and node.tag == idx
        for (idx, item_id) in &frame.item_ids {
            let node = tree.get(*item_id).expect("Item node must exist");
            assert_eq!(node.role, WidgetRole::DropdownItem);
            assert_eq!(node.layer, UiLayer::Popup);
            assert_eq!(node.tag, *idx as u64);
        }

        // Verify hit_test_target retrieves the second item with tag == 1
        let hit_target = tree
            .hit_test_target(Point::new(220.0, 160.0))
            .expect("Item must be hit");
