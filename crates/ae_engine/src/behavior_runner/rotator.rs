...
/// Updates continuous Y-axis rotation and Rapier physics body orientation for `PlayerTag` entities.
pub fn update_player_rotations(
    world: &mut World,
    physics_world: &mut PhysicsWorld,
    dt: f32,
    dirty_entities: &mut Vec<Entity>,
) {
    let mut player_tag_entities = Vec::new();
    for (ent, _tag) in world
        .query::<(hecs::Entity, &ae_core::ecs::PlayerTag)>()
        .iter()
    {
        player_tag_entities.push(ent);
    }
