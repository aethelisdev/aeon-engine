...
pub struct AnchorGuideParams<'a, F: Fn(f32, f32) -> Point> {
    ...
    /// Computed bounding rectangle of the target element in canvas local space.
    pub local_elem_rect: Rect,
    ...
    pub base_scale: f32,
    /// Closure mapping virtual canvas coordinates `(x, y)` to canvas local `Point`.
    pub to_local_pos: &'a F,
}
...
pub fn build_anchor_pin_and_guide<F: Fn(f32, f32) -> Point>(
    ...
    let anchor_p = (params.to_local_pos)(anchor_x, anchor_y);
    let elem_center = Point::new(
        params.local_elem_rect.x + params.local_elem_rect.width * 0.5,
        params.local_elem_rect.y + params.local_elem_rect.height * 0.5,
    );
    ...
    if line_w > 1.0 {
        ...
        scope.empty_box_passive_named("UiDesignerAnchorGuideH", h_line_style);
    }
    ...
    if line_h > 1.0 {
        let v_line_style = Style::new()
            .position_absolute()
            .left(elem_center.x - 0.5)
            .top(min_y)
            .width(1.0)
            .height(line_h)
            .background(guide_color);
        scope.empty_box_passive_named("UiDesignerAnchorGuideV", v_line_style);
    }

    // ── 3. Anchor Origin Pin ──────────────────────────────────────────────────
    let pin_radius = (4.5 * params.base_scale.max(0.8)).clamp(3.0, 8.0);
    let pin_style = Style::new()
        .position_absolute()
        .left(anchor_p.x - pin_radius)
        .top(anchor_p.y - pin_radius)
    ...
    scope.empty_box_passive_named("UiDesignerAnchorPin", pin_style);
    ...
    let label_font_size = (9.0 * params.base_scale.max(0.75)).clamp(8.0, 12.0);
    ...
    scope.label_styled_passive(
        "UiDesignerAnchorLabel",
        format!("⚓ {}", anchor_name),
        label_font_size,
        Color::rgba(1.0, 0.85, 0.35, 0.95),
        TextAlign::Left,
        label_style,
    );
}