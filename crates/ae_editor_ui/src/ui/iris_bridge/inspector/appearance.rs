...
pub fn build_appearance_card(
    ...
    let padding = 8.0;
    let card_h = 24.0 + 24.0 + 24.0 + 24.0 + padding * 2.0 + 6.0;
    ...
    if let Some(node) = tree.get_mut(hex_txt_id) {
        ...
        let hex_val = if ctx.params.inspector_color_hex.is_empty() {
            "#6699cc"
        } else {
    ...
    // 5. Row 3: 7 Distinct Color Swatches: White, Gray, Black, Red, Green, Blue, Yellow
    let default_palette = [
    ...
    // Render default 7 palette swatches
    for (i, &col) in default_palette.iter().enumerate() {
        ...
        if let Some(node) = tree.get_mut(sw_id) {
            ...
            let border_col = if is_hovered {
                Color::rgba(0.80, 0.85, 0.95, 0.95)
            } else {
                Color::rgba(0.212, 0.220, 0.259, 0.85)
            };
            node.style = Style::new()
