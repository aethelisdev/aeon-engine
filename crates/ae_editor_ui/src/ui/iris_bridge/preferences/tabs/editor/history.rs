...
pub fn build_history_card(
    ...
    let section = SettingSectionBuilder::new(card_rect, "📝  History Settings")
        .collapsed(is_hist_collapsed)
        .cursor_pos(Some(ctx.cursor_pos))
        .build(tree, parent_id);
    let hist_card_id = section.card_id;
    targets
        .section_toggles
        .push(("editor_history", section.header_rect));
